...
/**
 * GET - Get review statistics for a farmer
 */
export async function GET(
  ...
  try {
    ...
    if (reviews.length > 0 && productIds.length > 0) {
      // Re-query reviews with productId
      const reviewsWithProduct = await prisma.review.findMany({
        where: {
          productId: { in: productIds },
          status: "APPROVED",
          targetType: "PRODUCT",
      ...
      // Get product names for products with reviews
    ...
    let topProductResponse: { name: string; avgRating: number } | null = null;
    if (topProduct) {
